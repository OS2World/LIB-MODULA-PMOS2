...
FROM ScreenEditor IMPORT
    (* type *)  Structure,
    (* proc *)  CardinalField, RealField, MenuField, StringField, Combine,
                CreateField, MakeArray, ScreenEdit;
...
PROCEDURE RunTheTest;
    ...
    BEGIN
        ...
        *)
        ...
        Combine (R, StringField (stringvar, 4, 10, 8));
   ...
   (*   ScreenEdit (w, R, abort);   *)
        ...
        Combine (R, StringField (Text, 8, 3, 10));

        (* Add a menu field. *)

        menuvar := 2;
        Combine (R, MenuField (menuvar, 9, 5, 6, 30, CreateAMenu()));
