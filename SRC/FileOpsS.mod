...
PROCEDURE HideFile (name: ARRAY OF CHAR;  HideIt: BOOLEAN);
    ...
    BEGIN
        IF OS2.DosOpen (name, cid, Action, 0, 0, OpenFlags, Mode, NIL) = 0 THEN
...
PROCEDURE EndPosition (cid: ChanId): FilePos;
    ...
    BEGIN
        Actual := 0;  EndPos := 0;
        OS2.DosSetFilePtr (cid, 0, OS2.FILE_CURRENT, ADR(Actual));
        OS2.DosSetFilePtr (cid, 0, OS2.FILE_END, ADR(EndPos));
        OS2.DosSetFilePtr (cid, Actual, OS2.FILE_BEGIN, ADR(Actual));
        RETURN EndPos;
    END EndPosition;
...
PROCEDURE SetFileSize (cid: ChanId;  newsize: CARDINAL);
    ...
    BEGIN
        OS2.DosSetFileSize (cid, newsize);
    END SetFileSize;
